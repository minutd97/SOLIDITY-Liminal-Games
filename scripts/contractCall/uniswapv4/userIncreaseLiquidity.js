...
async function execute() {
    try {
        ...
        const [extraETH, extraLIM] = await V4PoolHelper.getAmountsForExact(ethers.parseEther("0.05"), 0);
        ...
        if (extraLIM > 0) {
            await LiminalToken.connect(user).approve(PERMIT2_ADDRESS, ethers.parseUnits("20000000000", 18));
            const expiration = Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 365;
            const MAX_ALLOW = (1n << 160n) - 1n;
            await permit2.approve(LIMINAL_TOKEN, POSITION_MANAGER, MAX_ALLOW, expiration);
        }
